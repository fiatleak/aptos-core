...
// TODO: import the proper type once globally defined.
type AggregatorID = u64;
        ...
        match self.versioned_map.entry(txn_idx) {
            Entry::Occupied(mut o) => {
                let bypass = match &**o.get() {
                    Value(_, maybe_delta) => {
                        maybe_delta.map_or(NoBypass, |delta| DeltaBypass(delta))
                    },
...
mod test {
    ...
    fn test_delta() -> DeltaOp {
        DeltaOp::new(DeltaUpdate::Plus(30), 1000, 30, 0)
    }

    fn negative_delta() -> DeltaOp {
        DeltaOp::new(DeltaUpdate::Minus(30), 1000, 0, 30)
    }
